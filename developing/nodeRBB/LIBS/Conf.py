...
class Conf:
    ...
    def check_semantic(self):
        ...
        for n in self.robot:
            if self.robot[n].has_key("-->"):
                sp = [self.node["name"] + "." +
                      x for x in self.robot[n]["-->"] if x.find(".") < 0]
    ...
    def dependency(self):
        dep_resueltas = [x for x in self.robot.keys(
        ) if get_field(self.robot[x], "-->") == []]
        condep = [x for x in self.robot.keys() if get_field(
            self.robot[x], "-->") != []]
        nivel_dep = 0
        while condep != [] and nivel_dep < 20:
            for i in condep:
                ...
                dep_imcump = [x for x in get_field(
                    self.robot[i], "-->") if x not in dep_resueltas]
    ...
    def local_remote(self, k):
        local = [x for x in self.robot[k]["-->"]
                 if x.find(self.node["name"] + ".") > -1]
        remote = [x for x in self.robot[k]["-->"]
                  if x.find(self.node["name"] + ".") == -1]
        return local, remote
    ...
    def with_local_deps(self):
        deps = [x for x in self.robot.keys() if get_field(
            self.robot[x], "-->") != [] and self.has_local(x)]
        return deps

    def with_remote_deps(self):
        deps = [x for x in self.robot.keys() if get_field(
            self.robot[x], "-->") != [] and self.has_remote(x)]
        # print deps
        return deps
