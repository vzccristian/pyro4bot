...
def import_class(list_class):
    try:
        list_class.append(("SERVICES", "URI_resolver",
                           "URI_resolver"))  # mejor cambiar
...
class NODERB (object):
    ...
    def load_robot(self):
        ...
        for k in self.N_conf.whithout_deps():
            self.start__object(k, self.ROBOT[k])
        ...
        while object_robot != []:
            ...
            self.ROBOT[k]["nr_local"], self.ROBOT[k]["nr_remote"] = self.N_conf.local_remote(
                k)
    ...
    def pyro4bot__object(self, d, proc_pipe):
        ...
        try:
            daemon = Pyro4.Daemon(host=ip, port=ports)
            uri = daemon.register(eval(d["cls"])(
                data=[], **d), objectId=name_ob)
